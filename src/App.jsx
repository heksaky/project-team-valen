...
export default function App() {
  ...
  const socialMedia = [
    ...
    { name: "LinkedIn", icon: "💼", link: "https://www.linkedin.com/in/Dhoni-Abdul-Rouf" },
    ...
    <div className={`${darkMode ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-800'} min-h-screen font-sans transition-colors duration-700`}>
      ...
      <nav className={`${darkMode ? 'bg-slate-800/80' : 'bg-blue-950/90'} backdrop-blur-md text-white p-4 sticky top-0 z-50 shadow-lg`}>
        <div className="max-w-6xl mx-auto flex justify-between items-center px-4">
          <h1 className="text-xl font-black text-amber-400 tracking-tighter italic">DHONI.PROFILE</h1>
          <div className="flex items-center space-x-4 md:space-x-8 text-sm font-bold">
            <button onClick={() => scrollToSection('home')} className="hover:text-amber-400 transition-all hidden md:block uppercase">Home</button>
            <button onClick={() => scrollToSection('tentang')} className="hover:text-amber-400 transition-all hidden md:block uppercase">About</button>
            <button 
      ...
      <header id="home" className={`${darkMode ? 'bg-gradient-to-b from-slate-800 to-slate-900' : 'bg-blue-900'} text-white pt-24 pb-32 text-center px-6 transition-all`}>
        <div className="max-w-3xl mx-auto">
          <div className="relative mb-8 flex justify-center group">
            ...
            <div className="w-44 h-44 bg-white rounded-full border-8 border-amber-400 flex items-center justify-center shadow-2xl overflow-hidden relative z-10 transform group-hover:scale-105 transition-transform duration-500">
          ...
          <div className="inline-block bg-white/10 backdrop-blur-md px-6 py-2 rounded-full border border-white/20 mb-6">
            <p className="text-lg font-bold uppercase tracking-widest">{kelas} — {prodi}</p>
          </div>
      ...
      <section id="tentang" className="max-w-4xl mx-auto px-6 py-16 -mt-20 relative z-10">
        <div className={`${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} p-8 md:p-12 rounded-[2.5rem] shadow-2xl border text-justify transition-all`}>
          ...
          <div className={`${darkMode ? 'text-slate-300' : 'text-slate-600'} leading-relaxed space-y-6 text-lg`}>
            <p>
              Halo, perkenalkan saya <span className="font-bold text-amber-500">{namaLengkap}</span>. Saya adalah individu yang disiplin, bertanggung jawab, dan memiliki kemampuan komunikasi yang baik. Saya terbiasa bekerja dalam tim maupun secara mandiri, serta mampu beradaptasi dengan lingkungan kerja yang dinamis.
            </p>
            <p>
              Dengan latar belakang pendidikan di <span className="font-semibold italic">Teknologi Informasi</span>, saya memiliki pengalaman dalam organisasi yang melatih kemampuan kepemimpinan dan manajemen waktu. Saya siap memberikan kontribusi terbaik di setiap bidang yang saya tekuni dengan semangat belajar yang tinggi dan cara berpikir kreatif.
            </p>
          </div>
        </div>
      </section>
      ...
      <footer id="contact" className={`${darkMode ? 'bg-black border-t border-slate-800' : 'bg-blue-950'} text-white py-24 px-6 mt-20 transition-all`}>
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-4xl font-black text-amber-400 mb-4 tracking-tighter">LET'S CONNECT!</h2>
          <p className="text-slate-400 mb-12 font-medium">Tertarik berkolaborasi? Hubungi saya di bawah ini.</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {socialMedia.map((socmed, index) => (
              <a key={index} href={socmed.link} target="_blank" rel="noopener noreferrer" 
                ...
                <div className="text-4xl mb-4 group-hover:scale-150 transition-all duration-300">{socmed.icon}</div>
          ...
          <div className="mt-24 pt-8 border-t border-white/5 text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em]">
            © 2026 {namaLengkap} — BUILT WITH REACT & TAILWIND
          </div>
        </div>
      </footer>
